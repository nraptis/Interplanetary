        ...
        do {
            if directoryURL.startAccessingSecurityScopedResource() {
                ...
                return fileURLs.compactMap { url in
                    if var string = url.absoluteString.removingPercentEncoding {
            ...
            directoryURL.stopAccessingSecurityScopedResource()
            print("Could not load directory \"\(directory)\"")
        }
        return [String]()
    }
    
    func getAllFilesRecursive(directory: String) -> [String] {
        let list = getAllFiles(directory: directory)
        var result = [String]()
        for path in list {
            let characters = Array(path)
            if characters.count > 0 {
                var isDirectory = false
                if characters[characters.count - 1] == "/" || characters[characters.count - 1] == "\\" {
                    isDirectory = true
                }
                if isDirectory {
                    let subpaths = getAllFilesRecursive(directory: path)
                    result.append(contentsOf: subpaths)
                } else {
                    result.append(path)
                }
            }

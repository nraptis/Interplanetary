...
//  Yo Mamma Be Ugly
//
//  Created by Nick Raptis on 11/11/23.
//
...
class FileBuffer {
    ...
    func copy(fileBuffer: FileBuffer) {
        if fileBuffer.length > capacity {
            reserveCapacity(minimumCapacity: fileBuffer.length)
        }
        readCursor = fileBuffer.readCursor
        writeCursor = fileBuffer.writeCursor
        length = fileBuffer.length
        for index in 0..<length {
            buffer[index] = fileBuffer.buffer[index]
        }
    }
    ...
    func save(filePath: String?) -> Bool {
        ...
        return save(data: fileData, filePath: filePath)
    }

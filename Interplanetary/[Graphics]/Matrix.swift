...
extension matrix_float4x4 {
    ...
    mutating func translation(point: Math.Point) {
    ...
    func process3d(point3: simd_float3) -> simd_float3 {
    ...
    func process2d(point3: simd_float3) -> simd_float3 {
        let x = columns.0.x * point3.x + columns.1.x * point3.y + columns.2.x * point3.z + columns.3.x
        let y = columns.0.y * point3.x + columns.1.y * point3.y + columns.2.y * point3.z + columns.3.y
        let z = columns.0.z * point3.x + columns.1.z * point3.y + columns.2.z * point3.z + columns.3.z
        return simd_float3(x, y, z)
    ...
    func process3d(point: Math.Point) -> Math.Point {
        let x = columns.0.x * point.x + columns.1.x * point.y + columns.3.x
        let y = columns.0.y * point.x + columns.1.y * point.y + columns.3.y
        let w = columns.0.w * point.x + columns.1.w * point.y + columns.3.w
        if fabsf(w) > Math.epsilon {
            let scale = 1.0 / w
            return Math.Point(x: x * scale, y: y * scale)
        } else {
            return Math.Point(x: x, y: y)
        }
    }
    
    func process2d(point: Math.Point) -> Math.Point {
        let x = columns.0.x * point.x + columns.1.x * point.y + columns.3.x
        let y = columns.0.y * point.x + columns.1.y * point.y + columns.3.y
        return Math.Point(x: x, y: y)
    }
    
    func process3d(point: Math.Point, screenWidth: Float, screenHeight: Float) -> Math.Point {
        var x = columns.0.x * point.x + columns.1.x * point.y + columns.3.x
        var y = columns.0.y * point.x + columns.1.y * point.y + columns.3.y
        let w = columns.0.w * point.x + columns.1.w * point.y + columns.3.w
        if fabsf(w) > Math.epsilon {
            let scale = 1.0 / w
            x *= scale
        ...
        x = screenWidth * (x + 1.0) * 0.5
        y = screenHeight * (1.0 - (y + 1.0) * 0.5)
        return Math.Point(x: x, y: y)
    }
    
    func process2d(point: Math.Point, screenWidth: Float, screenHeight: Float) -> Math.Point {
        var x = columns.0.x * point.x + columns.1.x * point.y + columns.3.x
        var y = columns.0.y * point.x + columns.1.y * point.y + columns.3.y
        x = screenWidth * (x + 1.0) * 0.5
        y = screenHeight * (1.0 - (y + 1.0) * 0.5)

...
class Graphics {
    ...
    @MainActor func loadTexture(url: URL) -> MTLTexture? {
        ...
        return loadTexture(nsImage: nsImage)
    }
    ...
    @MainActor func loadTexture(nsImage: NSImage?) -> MTLTexture? {
    ...
    func buffer<Element>(array: [Element]) -> MTLBuffer? {
        guard let metalDevice = metalDevice else {
            return nil
        }
    ...
    func write<Element>(buffer: MTLBuffer, array: [Element]) {
        let length = MemoryLayout<Element>.stride * array.count
